class RedisManager {
  constructor() {
    ...
    const redisConfig = {
      ...
      enableOfflineQueue: true, // ✅ ENABLE QUEUE (FIX LỖI)
      connectTimeout: 10000, // ✅ TĂNG TIMEOUT
      keepAlive: 1,
      noDelay: true,
      retryStrategy: (times) => {
        if (times > 10) return null; // ✅ Retry nhiều hơn
  ...
  setupEventHandlers() {
    ...
    this.publisherClient.on('connect', () => {
      log(colors.green, 'REDIS', colors.reset, 'Publisher connecting...');
    });

    this.publisherClient.on('ready', () => {
      this.isPublisherReady = true;
      log(colors.green, 'REDIS', colors.reset, '✅ Publisher READY');
    });
    ...
    this.publisherClient.on('close', () => {
      log(colors.yellow, 'REDIS', colors.reset, '⚠️ Publisher closed');
      this.isPublisherReady = false;
    });

    this.publisherClient.on('reconnecting', () => {
      log(colors.yellow, 'REDIS', colors.reset, 'Publisher reconnecting...');
      this.isPublisherReady = false;
    });

    // ✅ SUBSCRIBER EVENTS
    this.subscriberClient.on('connect', () => {
      log(colors.green, 'REDIS', colors.reset, 'Subscriber connecting...');
    });

    this.subscriberClient.on('ready', () => {
      this.isSubscriberReady = true;
      log(colors.green, 'REDIS', colors.reset, '✅ Subscriber READY');
    });
    ...
    this.subscriberClient.on('close', () => {
      log(colors.yellow, 'REDIS', colors.reset, '⚠️ Subscriber closed');
      this.isSubscriberReady = false;
    });

    this.subscriberClient.on('reconnecting', () => {
      log(colors.yellow, 'REDIS', colors.reset, 'Subscriber reconnecting...');
      this.isSubscriberReady = false;
    });
  ...
  async subscribe(channel, callback) {
    try {
      ...
      if (!ready) {
        throw new Error('Subscriber not ready after timeout');
      }
  ...
  async publish(channel, message) {
    try {
      ...
      if (!ready) {
        throw new Error('Publisher not ready after timeout');
      }
  ...
  async scanKeys(pattern) {
    ...
    const ready = await this._ensurePublisherReady();
    if (!ready) {
      log(colors.red, 'REDIS', colors.reset, 'Cannot scan: client not ready');
      return [];
    }
  ...
  async saveBrokerData(broker, data) {
    ...
    try {
      ...
      const ready = await this._ensurePublisherReady();
      if (!ready) {
        log(colors.red, 'REDIS', colors.reset, `Cannot save ${broker}: Redis not ready`);
        return { 
          success: false, 
  ...
  async getBroker(brokerName) {
    try {
      ...
      const ready = await this._ensurePublisherReady();
      if (!ready) {
        log(colors.red, 'REDIS', colors.reset, 'Cannot get broker: Redis not ready');
        return null;
      }
  ...
  async getAllBrokers() {
    try {
      ...
      const ready = await this._ensurePublisherReady();
      if (!ready) {
        log(colors.red, 'REDIS', colors.reset, 'Cannot get all brokers: Redis not ready');
        return [];
      }
  ...
  // ================================================================
  // REST OF FUNCTIONS (thêm connection check cho các hàm quan trọng)
  // ================================================================
  ...
  // Reset tracking functions (giữ nguyên)
